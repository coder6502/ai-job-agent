...
export default async function handler(req, res) {
  ...
  // Retry on transient overload (503) — Google's servers being busy, not a config error.
  // Fail fast on 429 (quota exhausted) since retrying that won't help within the time budget.
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (response.status === 503) {
        ...
        console.error(`Gemini overloaded (attempt ${attempt + 1}/3):`, lastErrText);
        if (attempt < 2) {
      ...
      if (response.status === 429) {
        ...
        console.error('Gemini quota exhausted, not retrying:', errText);
        return res.status(429).json({
          error: 'AI service is temporarily at capacity (daily quota reached). Please try again later.'
        });
      }
      ...
      console.error(`Chat handler attempt ${attempt + 1}/3 crashed:`, err);
      lastErrText = err.message;
    }
  }

  // All retries exhausted on transient overload
  return res.status(503).json({
    error: 'AI service is experiencing high demand right now. Please try again in a moment.',
    detail: lastErrText
  });
}
